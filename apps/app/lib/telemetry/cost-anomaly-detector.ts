...
export class CostAnomalyDetector {
  ...
  async analyzeRequest(request: {
    ...
    try {
      ...
      // Frequency anomaly
      const frequencyAnomaly = await this.detectFrequencyAnomaly(request.userId, request.timestamp);
      if (frequencyAnomaly) anomalies.push(frequencyAnomaly);

      // Pattern anomaly (unusual model usage)
      const patternAnomaly = this.detectPatternAnomaly(request.userId, request.model);
  ...
  private async detectFrequencyAnomaly(userId: string, timestamp: Date): Promise<Anomaly | null> {
  ...
  private updateBaseline(
    ...
    baseline.avgTokens =
      baseline.avgTokens * (1 - alpha) + (request.inputTokens + request.outputTokens) * alpha;
