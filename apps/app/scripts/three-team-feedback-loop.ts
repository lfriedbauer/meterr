...
- Team attribution built-in`;
    ...
    writeFileSync(path.join(this.outputDir, 'design-document.md'), designDoc);
  }
...
volumes:
  ...
  async researchValidationPhase() {
    ...
    const artifacts = this.currentCycle.artifacts.map((a) => `${a.name}: ${a.type}`).join('\n');
  ...
  async saveCycleResults() {
    const results = {
      ...
      summary: {
        ...
        criticalFeedback: this.currentCycle.feedback.filter((f) => f.severity === 'critical')
          .length,
  ...
  async runMultipleCycles(numCycles: number = 3) {
    for (let i = 0; i < numCycles; i++) {
      ...
      this.currentCycle.feedback = this.currentCycle.feedback.filter(
        (f) => f.actionRequired && f.severity === 'critical'
      );
  ...
  async generateFinalReport() {
    ...
    this.currentCycle.nextSteps.forEach((step) => console.log(`□ ${step}`));
