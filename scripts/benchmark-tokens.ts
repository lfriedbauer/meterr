...
const SAMPLE_TEXTS = [
  ...
  Array(1000).fill('This is a test sentence. ').join(''),
  // Add a very long text (simulating a large document)
  Array(10000).fill('This is a test sentence. ').join('')
];
...
async function benchmarkGPU() {
  ...
  try {
    ...
    for (let i = 0; i < SAMPLE_TEXTS.length; i++) {
      ...
      console.log(`  Time: ${duration}ms`);
      console.log(`  Speed: ${(SAMPLE_TEXTS[i].length / duration * 1000).toFixed(2)} chars/second`);
    }
