...
async function main() {
  ...
  for (const [fileName, docsFile] of docsAnalysis) {
    if (claudeAnalysis.has(fileName)) {
      ...
      if (docsFile.hash === claudeFile.hash) {
        ...
        if (similarity > 10) { // More than 10% similar
          similarContent.push({
            file: fileName,
  ...
  // Calculate totals
  const totalDocsLines = Array.from(docsAnalysis.values())
    .reduce((sum, f) => sum + f.lines, 0);
  const totalClaudeLines = Array.from(claudeAnalysis.values())
    .reduce((sum, f) => sum + f.lines, 0);
  ...
  const complexity: OverlapAnalysis['estimatedConsolidationTime']['complexity'] = 
    overlappingFiles > 10 ? 'high' : 
    overlappingFiles > 5 ? 'medium' : 'low';
  ...
  console.log('\n📈 SCOPE:');
  console.log(`  /docs:           ${analysis.totalFiles.docs} files (${analysis.totalLines.docs} lines)`);
  console.log(`  /.claude/context: ${analysis.totalFiles.claude} files (${analysis.totalLines.claude} lines)`);
  console.log(`  Total:           ${analysis.totalFiles.docs + analysis.totalFiles.claude} files (${analysis.totalLines.docs + analysis.totalLines.claude} lines)`);
  ...
  if (analysis.uniqueToDocs.length > 0) {
    console.log(`    ${analysis.uniqueToDocs.slice(0, 3).join(', ')}${analysis.uniqueToDocs.length > 3 ? '...' : ''}`);
  }
  console.log(`  Only in /.claude/context: ${analysis.uniqueToClaude.length} files`);
  if (analysis.uniqueToClaude.length > 0) {
    console.log(`    ${analysis.uniqueToClaude.slice(0, 3).join(', ')}${analysis.uniqueToClaude.length > 3 ? '...' : ''}`);
  }
  ...
  console.log(`  Complexity:       ${analysis.estimatedConsolidationTime.complexity.toUpperCase()}`);
  ...
  const reportPath = path.join(process.cwd(), 'research', 'documentation-analysis', 'overlap-analysis.json');
