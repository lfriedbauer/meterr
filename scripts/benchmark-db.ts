...
async function runBenchmark() {
  ...
  for (const size of batchSizes) {
    ...
    db.run(
      `INSERT INTO tokens (id, user_id, provider, model, input_tokens, output_tokens, cost) VALUES ${values}`
    );
    const duration = Date.now() - start;

    console.log(
      `  ${size} rows: ${duration}ms (${((size / duration) * 1000).toFixed(0)} inserts/second)`
    );
  }
