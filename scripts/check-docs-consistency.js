...
async function main() {
  ...
  console.log(
    `${colors.gray}Found ${docFiles.length} documentation files to check${colors.reset}\n`
  );
  ...
  for (const file of docFiles) {
    ...
    const providerPattern =
      /\b(openai|anthropic|google|azure|mistral|cohere|bedrock|hugging face|replicate|together|groq)\b/gi;
    const mentionedProviders = content.match(providerPattern);
    if (mentionedProviders) {
      const uniqueProviders = [
        ...new Set(mentionedProviders.map((p) => p.toLowerCase().replace(' ', '_'))),
      ];
      uniqueProviders.forEach((provider) => {
        const normalizedProvider = provider
          .replace('hugging face', 'hugging_face')
          .replace('azure', 'azure_openai')
          .replace('bedrock', 'aws_bedrock')
          .replace('together', 'together_ai');
        if (
          !allProviders.some(
            (p) => normalizedProvider.includes(p) || p.includes(normalizedProvider)
          )
        ) {
          warnings.push(
            `${file}: References provider "${provider}" not in supported_providers list`
          );
        }
      });
    }
    ...
    if (content.includes('Next.js')) {
      ...
      matches.forEach((match) => {
        if (match[1] && !sharedData.tech_stack.frontend.includes(match[1])) {
          errors.push(
            `${file}: Next.js version "${match[1]}" doesn't match shared data "${sharedData.tech_stack.frontend}"`
          );
        }
      });
    }
    ...
    const timeReferences =
      /\b(week\s+\d+|month\s+\d+|Q[1-4]\s+20\d{2}|sprint\s+\d+|by\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)|end\s+of\s+(week|month|quarter|year))\b/gi;
    const timeMatches = content.match(timeReferences);
    if (timeMatches) {
      timeMatches.forEach((match) => {
        errors.push(
          `${file}: Contains forbidden time reference "${match}" - use phase-based planning`
        );
      });
    }
    ...
    if (content.toLowerCase().includes('phase')) {
      ...
      phaseMatches.forEach((match) => {
        ...
        const expectedPhase = sharedData.methodology.phases.find((p) =>
          p.name.includes(`Phase ${match[1]}`)
        );
        if (expectedPhase && expectedPhase.confidence !== `${confidence}%`) {
          warnings.push(
            `${file}: Phase ${match[1]} confidence "${confidence}%" doesn't match expected "${expectedPhase.confidence}"`
          );
        }
      });
    }
    ...
    if (content.includes('Pay What You Save') || content.includes('pricing')) {
      ...
      matches.forEach((match) => {
        ...
        if (percentage !== parseInt(sharedData.pricing.percentage)) {
          errors.push(
            `${file}: Pricing percentage "${percentage}%" doesn't match shared data "${sharedData.pricing.percentage}"`
          );
        }
      });
    }
  ...
  console.log(
    `  • Providers: ${sharedData.supported_providers.primary.length} primary, ${sharedData.supported_providers.secondary.length} secondary`
  );
